...
/*
    ...
    이렇게 해서 각도를 구해주면 비로소 135도와 225도에 해당하는 라디안 값들을 얻게 되는데, 
    x좌표는 위에 언급한바와 같이 135도, 225도 둘 다 같지만, 
    y좌표의 경우 sin(135º) = sin(pi - 45º) = sin45º / sin(225º) = sin(pi + 45º) = -sin45º 로 y좌표의
    부호가 서로 달라지게 되어 이를 총알의 움직임에 사용하면, 타겟이 위아래로 움직이건 간에 정확히 타겟을 향하게 된다.
*/
float GetAngle( FPOINT origin, FPOINT target )
{
